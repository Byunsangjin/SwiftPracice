...
class RevealViewController: UIViewController {
    ...
    var isSideBarSowing = false // 현재 사이드 바가 열려 있는지 여부
    ...
    func setupView() {
        ...
        if let vc = self.storyboard?.instantiateViewController(withIdentifier: "sw_front") as? UINavigationController {
            ...
            self.view.addSubview(vc.view) // _프론트 컨트롤러의 뷰를 메인 컨트롤러의 서브 뷰로 등극
    ...
    func setShadowEffect(shadow: Bool, offset: CGFloat) {
        if (shadow == true) { // 그림자 효과 설정
            ...
            self.contentVC?.view.layer.shadowOffset = CGSize(width: offset, height: offset) // 그림자
        } else {
    ...
    func openSideBar(_ complete: ( () -> Void)? ) {
        ...
        UIView.animate(
            ...
            completion: {
                if $0 == true {
                    self.isSideBarSowing = true // 열림 상태로 플래그를 변경한다.
                    complete?()
                }
        }
        )
