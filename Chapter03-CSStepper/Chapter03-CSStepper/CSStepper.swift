...
class CSStepper : UIControl {
    ...
    public var centerLabel = UILabel() // 중앙 라벨
    ...
    private func setup() {
        let borderWidth : CGFloat = 0.5 // 테두리 두께 값
        ...
        self.centerLabel.font = UIFont.boldSystemFont(ofSize: 16) // 레이블 폰트
        self.centerLabel.textAlignment = .center // 가운데 정렬
        self.centerLabel.backgroundColor = bgColor // 배경 색상 설정
        self.centerLabel.layer.borderWidth = borderWidth // 레이블 두께 설정
        self.centerLabel.layer.borderColor = borderColor // 레이블 테두리 색상 설정
    ...
    @objc public func valueChange(_ sender : UIButton) {
        ...
        // 더한 결과가 최대값보다 크거나 최소값보다 작으면 값을 변경하지 않고 종료
        if sum > self.maximumValue || sum < self.minimumValue{
            return
        }
        
        // 현재 value 값에 -1 or 1을 한다.
